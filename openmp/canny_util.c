...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma,
                       short int **smoothedim)
{
    int r, c, rr, cc;   /* Loop counters. */
    int windowsize;     /* Kernel dimension. */
    int center;         /* Half of the windowsize. */
    float *tempim;      /* Temporary buffer image. */
    float *kernel;      /* 1D Gaussian kernel. */
    float dot, sum;     /* Accumulators. */

    /**************************************************************************
     * Create a 1D Gaussian smoothing kernel.
     **************************************************************************/
    if (VERBOSE) printf("   Computing the gaussian smoothing kernel.\n");
    make_gaussian_kernel(sigma, &kernel, &windowsize);
    center = windowsize / 2;
    ...
    *smoothedim = (short int *) calloc(rows * cols, sizeof(short int));
    ...
    /**************************************************************************
     * Blur in the X–direction using OpenMP.
     **************************************************************************/
    if (VERBOSE) printf("   Bluring the image in the X-direction.\n");
    #pragma omp parallel for private(c, cc, dot, sum) schedule(static)
    for (r = 0; r < rows; r++) {
        for (c = 0; c < cols; c++) {
            dot = 0.0;
            sum = 0.0;
    ...
    /**************************************************************************
     * Blur in the Y–direction using OpenMP.
     **************************************************************************/
    if (VERBOSE) printf("   Bluring the image in the Y-direction.\n");
    #pragma omp parallel for private(c, rr, dot, sum) schedule(static)
    for (r = 0; r < rows; r++) {
        for (c = 0; c < cols; c++) {
            dot = 0.0;
            sum = 0.0;
