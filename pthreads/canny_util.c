...
/* Structure for x-direction blur thread arguments */
struct thread_args_x {
    unsigned char *image;
    int rows;
    int cols;
    int col_s;         /* starting column (inclusive) */
    int col_e;         /* ending column (exclusive) */
    int center;
    float *kernel;
    float *tempim;
};

/* Thread function for x-direction blur */
void *blur_x(void *arguments)
{
    ...
    for (r = 0; r < args->rows; r++) {
        for (c = args->col_s; c < args->col_e; c++) {
            dot = 0.0;
            sum = 0.0;
            for (cc = -args->center; cc <= args->center; cc++) {
                if (((c + cc) >= 0) && ((c + cc) < args->cols)) {
                    dot += (float) args->image[r * args->cols + (c + cc)] *
                           args->kernel[args->center + cc];
...
/* Structure for y-direction blur thread arguments */
struct thread_args_y {
    float *tempim;
    int rows;
    int cols;
    int row_s;         /* starting row (inclusive) */
    int row_e;         /* ending row (exclusive) */
    int center;
    float *kernel;
    short int *smoothedim;
};

/* Thread function for y-direction blur */
void *blur_y(void *arguments)
{
    struct thread_args_y *args = (struct thread_args_y *) arguments;
    int r, c, rr;
    float dot, sum;
    for (r = args->row_s; r < args->row_e; r++) {
        for (c = 0; c < args->cols; c++) {
            dot = 0.0;
            sum = 0.0;
            for (rr = -args->center; rr <= args->center; rr++) {
                if (((r + rr) >= 0) && ((r + rr) < args->rows)) {
                    dot += args->tempim[(r + rr) * args->cols + c] *
                           args->kernel[args->center + rr];
                    sum += args->kernel[args->center + rr];
                }
            }
            args->smoothedim[r * args->cols + c] =
                       ...
                       short int **smoothedim)
{
    ...
    *smoothedim = (short int *) calloc(rows * cols, sizeof(short int));
    ...
    /**************************************************************************
     * X–direction blur using pthreads.
     **************************************************************************/
    if (VERBOSE) printf("   Bluring the image in the X-direction.\n");
    ...
    for (i = 0; i < N_T; i++) {
        ...
        args_x[i].rows = rows;
        args_x[i].cols = cols;
        args_x[i].col_s = i * col_per_t;
        args_x[i].col_e = (i == N_T - 1) ? cols : (i + 1) * col_per_t;
        args_x[i].center = center;
        args_x[i].kernel = kernel;
        args_x[i].tempim = tempim;
        pthread_create(&threads[i], NULL, blur_x, &args_x[i]);
    }
    for (i = 0; i < N_T; i++) {
    ...
    /**************************************************************************
     * Y–direction blur using pthreads.
     **************************************************************************/
    if (VERBOSE) printf("   Bluring the image in the Y-direction.\n");
    int rows_per_t = rows / N_T;
    for (i = 0; i < N_T; i++) {
        args_y[i].tempim = tempim;
        args_y[i].rows = rows;
        args_y[i].cols = cols;
        args_y[i].row_s = i * rows_per_t;
        args_y[i].row_e = (i == N_T - 1) ? rows : (i + 1) * rows_per_t;
        args_y[i].center = center;
        args_y[i].kernel = kernel;
        args_y[i].smoothedim = *smoothedim;
        pthread_create(&threads_y[i], NULL, blur_y, &args_y[i]);
    }
    for (i = 0; i < N_T; i++) {
        pthread_join(threads_y[i], NULL);
    }
