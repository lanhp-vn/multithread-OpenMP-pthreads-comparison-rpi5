...
//Added by Lan Pham for multi-thread 
struct thread_args_y
{
    float *tempim;         // Input image after x-blur (temporary buffer)
    int rows;              // Total number of rows in the image
    int cols;              // Total number of columns in the image
    int row_s;             // Starting row (inclusive) for this thread
    int row_e;             // Ending row (exclusive) for this thread
    int center;            // Half the kernel width
    float *kernel;         // Pointer to the 1D Gaussian kernel
    short int *smoothedim; // Output (smoothed) image buffer
};
...
void *blur_y(void *arguments)
{
   ...
   for(r = args->row_s; r < args->row_e; r++){
      for(c = 0; c < args->cols; c++){
         sum = 0.0;
         dot = 0.0;
         /* For each pixel, perform the y-direction convolution */
        ...
        short int **smoothedim)
{
   ...
   for(i = 0; i < N_T; i++)
   {
      ...
      args[i].cols = cols;
      args[i].col_s = col_per_t * i;
      args[i].col_e = (i == N_T - 1) ? cols : col_per_t * (i + 1);
      args[i].center = center;
      args[i].kernel = kernel;
      args[i].tempim = tempim;
      iret[i] = pthread_create(&thread[i], NULL, blur_x, &args[i]);
   ...
   for(i = 0; i < N_T; i++){
       ...
       args_y[i].rows = rows;
       args_y[i].cols = cols;
       args_y[i].row_s = rows_per_t * i;
       args_y[i].row_e = (i == N_T - 1) ? rows : rows_per_t * (i + 1);
