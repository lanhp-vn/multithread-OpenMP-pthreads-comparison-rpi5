...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma,
        short int **smoothedim)
{
   int r, c, rr, cc;    /* Loop counter variables. */
   ...
   /* Blur in the X-direction. */
   /*--------------------------*/
   int n_thread = 4;  // Number of threads to use
   omp_set_dynamic(0);
   ...
   /*---------------------------------------------------*/
   /* Blur in the Y-direction (Parallelized with OpenMP) */
   /*---------------------------------------------------*/
   if(VERBOSE) printf("   Bluring the image in the Y-direction.\n");
   // Here we choose to parallelize the outer loop (over rows).
   #pragma omp parallel for private(r, c, sum, dot, rr)
   for(r = 0; r < rows; r++){
      for(c = 0; c < cols; c++){
         sum = 0.0;
         dot = 0.0;
